...
def test_get_api_key_for_not_valid_email_and_password(
    ...
    """ Проверяем, что запрос api ключа с неверным email пользователя
        возвращает статус 403 и в результате нет слова key"""
    status, result = pf.get_api_key(email, password)
    assert status == 403
...
def test_add_new_pet_with_valid_data(
    name='Отто Фриш',
    animal_type='Кот Обыкновенный',
    age='4',
    pet_photo_path='images/Cat_Photo.jpg'
):
...
def test_add_new_pet_with_space_in_age(name='Отто Фриш', animal_type='Кот Обыкновенный', age=' ', pet_photo_path='images/Cat_Photo.jpg'):
    """ Проверяем, что запрос на добавление нового питомца
        с пустым полем возраста выполняется успешно"""
    pet_photo_path = os.path.join(os.path.dirname(__file__), pet_photo_path)
...
def test_successful_delete_self_pet():
    ...
    if not my_pets['pets']:
        pf.add_new_pet(auth_key, "Виктор", "Кот Необыкновенный", "1", "images/Cat_Photo.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, pf.MY_PETS)
...
def test_successful_update_self_pet_info(name='Генрих', animal_type='Кот Обычный', age=1):
...
def test_rejection_update_self_pet_info_without_name(name='', animal_type='попугай', age=2):
...
def test_rejection_update_self_pet_info_without_animal_type(name='Евклид', animal_type='', age=5):
...
def test_succsessful_update_self_pet_info_with_spase_name(name=' ', animal_type='крокодил', age=1):
...
def test_add_new_pet_with_valid_data_without_foto(name='Вильгельм', animal_type='Кот Простой', age='1'):
...
def test_add_new_pet_with_incorrect_data_without_foto(name='№@#!&^*', animal_type='', age=''):
    """ Проверяем, что запрос на добавление нового питомца без фото с некорректно указанными параметрами
        name задаётся спецсимволами, animal_type и age - пустые, выполняется успешно.""" 
...
def test_successful_add_photo_of_pet(pet_id='', pet_photo_path='images/Cat_Photo.jpg'):
    ...
    status, result = pf.add_photo_for_pet(auth_key, pet_id, pet_photo_path)
